...
export default function FormProfile() {
  ...
  async function submitHandler(e: any) {
    ...
    const submitRes = await fetch("/api/updateProfile", {
      method: "POST",
      headers: {
    ...
    if (submitData.error) {
      setFormFeedback(submitData.error);
    } else {
      setFormFeedback("Perfil atualizado com sucesso!");
    }
      ...
      >
        ...
        <div className="flex justify-center items-center mt-10">
          ...
          <button
            ...
            type="submit"
            className={"w-9/12 bg-indigo-500 hover:bg-indigo-400 text-white rounded-lg px-4 py-3 mt-6"}
          >
            Salvar
          </button>
        </div>
